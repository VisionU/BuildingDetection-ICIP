...
void by_get_edges(unsigned char **out,int **in,int radius,int larger_equal,int h,int w);
...
inline void image_copy(unsigned char***out,unsigned char***in,int h,int w,int d){memcpy(out[0][0],in[0][0],sizeof(char)*h*w*d);}
...
inline void qx_label_2_rgb(unsigned char***labels_ppm,int**labels,int nr_labels,int h,int w)
{
	div_t div1, div2;
	int y,x;
...
void file_save_ascii_1d(char *file_path,int *out,int h,int w);
