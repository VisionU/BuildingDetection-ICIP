...
void SEEDS::LAB2RGB(float L, float a, float b, int* R, int* G, int* B)
{
	...
	fY = YT * powf(fY,1.0/3.0) + (!YT) * (7.787 * fY + 16.0/116.0);

	float fX = a / 500.0 + fY;
	bool XT = fX > T2;
	float X = (XT * powf(fX, 3) + (!XT) * ((fX - 16.0/116.0) / 7.787));

	float fZ = fY - b / 200.0;
	bool ZT = fZ > T2;
	float Z = (ZT * powf(fZ, 3) + (!ZT) * ((fZ - 16.0/116.0) / 7.787));
...
int SEEDS::RGB2LAB(const int& r, const int& g, const int& b, float* lval, float* aval, float* bval)
{
	...
	fX = XT * powf(xVal,1.0/3.0) + (!XT) * (7.787 * xVal + 16.0/116.0);

	// Compute L
	float Y3 = powf(yVal,1.0/3.0); 
	fY = YT*Y3 + (!YT)*(7.787*yVal + 16.0/116.0);
	lVal  = YT * (116 * Y3 - 16.0) + (!YT)*(903.3*yVal);

	fZ = ZT*powf(zVal,1.0/3.0) + (!ZT)*(7.787*zVal + 16.0/116.0);
...
int SEEDS::RGB2LAB_special(int r, int g, int b, float* lval, float* aval, float* bval)
{
	...
	fX = XT * powf(xVal,1.0/3.0) + (!XT) * (7.787 * xVal + 16/116);
	...
	fZ = ZT*powf(zVal,1.0/3.0) + (!ZT)*(7.787*zVal + 16/116);
