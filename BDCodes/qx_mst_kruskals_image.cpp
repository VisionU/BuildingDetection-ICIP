...
inline void qx_mst_compute_edges_per_pixel(int**edges,unsigned char*distance,unsigned char*image,int nr_channel,int &nr_edge,int y0,int x0,int yt,int xt,int h,int w)
{
	...
	//distance[nr_edge]=255-distance[nr_edge];
	nr_edge++;
}
...
//int qx_mst_kruskals_image::mst_(int*in_id_edge,int**in_edge,int in_nr_edge,unsigned char*edge_weight) 
//{ 
//	m_id_edge=in_id_edge; 
//	m_edge=in_edge; 
//	m_nr_edge=in_nr_edge; 
//	m_distance=edge_weight;
//	qx_sort_increase_using_histogram(m_id_edge,m_distance,m_nr_edge); 
...
int qx_mst_kruskals_image::mst(unsigned char*image,bool print_edges)
{
	...
	memcpy(m_image,image,sizeof(char)*m_h*m_w*m_nr_channel);
...
int qx_mst_kruskals_image::build_tree()
{
	...
	while(m_queue.length>0)
	{
		...
		for(int i=0;i<nr_connected;i++)
		{
			...
			{
				check.push_back(potential_child);
